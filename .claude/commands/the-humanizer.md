...
**HARD OUTPUT RULE: No em dashes (—) in the rewrite. Ever.** This applies regardless of what the source content uses or what punctuation feels stylistically appropriate. Replace every em dash with a comma, period, colon, or parenthetical based on context. This is a non-negotiable constraint, not a stylistic preference. Before presenting the rewrite, scan it for em dashes and replace any that slipped through.
